...
class UserService
{
    ...
    public function getUsers(string $type): Object
    {
        if($type=='admin')
        {
            return User::where('is_admin','1')->where('id','<>','4')->get();
        }
        if($type=='customer'){
    ...
    public function createCustomer(string $id,array $userData2,string $imageUrl): Customer
    {
        $insert2 = [
            'user_id'    => $id,
    ...
    public function updateCustomer(string $id,array $userData2, string $imageUrl = null): void
    {
