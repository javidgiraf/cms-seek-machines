          ...
          <form method="post" enctype="multipart/form-data" action="{{route('sellmachines.update',$sellmachine->id) }}">
            ...
            <div class="card">
              ...
              <div class="body">
                ...
                <div class="row clearfix">
                  <div class="col-lg-3 col-md-3 col-sm-3 form-control-label">
                    <label for="country">Available Country</label>
                  </div>
                  <div class="col-lg-9 col-md-9 col-sm-9">
                    <div class="form-group">
                      <select class="form-control show-tick ms sel-country" name="available_country">
                        <option value="">--Please Select--</option>
                        @foreach ($countries as $country)
                        <option value="{{ $country->id }}" {{$sellmachine->available_country == $country->id? "selected": ""}}>
                          {{ $country->name }}
                        </option>
                        @endforeach
                      </select>
                ...
                <div class="row clearfix">
                  <div class="col-lg-3 col-md-3 col-sm-3 form-control-label">
                    <label for="agent_id">Assigned to</label> <!-- Ensure the 'for' attribute matches the select's id -->
                  </div>
                  <div class="col-lg-9 col-md-9 col-sm-9">
                    <div class="form-group">
                      <select class="form-control show-tick ms sel-country" name="agent_id" id="agent_id" required>
                        <option value="" disabled {{ old('agent_id', $sellMachineAgent->agent_id ?? '') ? '' : 'selected' }}>--Please Select--</option> <!-- Placeholder -->
                        @if ($seekAgentUsers->isNotEmpty()) <!-- Check if there are any agents -->
                        @foreach ($seekAgentUsers as $data)
                        <option value="{{ $data->id }}" {{ (old('agent_id', $sellMachineAgent->agent_id ?? '') == $data->id) ? 'selected' : '' }}>
                          {{ $data->name }}
                        </option>
                        @endforeach
                        @else
                      ...
                      @error('agent_id') <!-- Error message for validation -->
